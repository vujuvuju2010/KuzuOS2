...
/* KuzuOS: Use gettimeofday-based monotonic timers with no threading */
/* static tor_mutex_t monotime_lock; - removed, not needed for single-threaded */

/** Initialize the monotonic timer subsystem. */
static void
monotime_init_internal(void)
{
  /* KuzuOS: No mutex needed in single-threaded mode */
  tor_assert(!monotime_initialized);
}
...
monotime_coarse_to_stamp(const monotime_coarse_t *t)
{
  ...
  /* KuzuOS: Fixed bug - use usec instead of undefined nsec */
  return (sec * STAMP_TICKS_PER_SECOND) | (usec >> 10);
}
...
monotime_add_msec(monotime_t *out, const monotime_t *val, uint32_t msec)
{
  ...
  /* KuzuOS: Fixed bug - use tv_usec instead of tv_nsec */
  out->tv_.tv_usec = val->tv_.tv_usec + (msec_remainder * 1000);
