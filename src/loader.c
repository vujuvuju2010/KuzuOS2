...
#define PAGE_SIZE       8192 // deafult is 4096 if changed then thee shall fix it back to 4096 if necesarry
...
void z_entry(unsigned long *sp, void (*fini)(void))
{
        ...
        for (i = 0; i < 2; i++, ehdr++) {
                ...
                phdr = (Elf_Phdr *)kmalloc(sz);
                if (!phdr) {
                        z_close(fd);
                        z_errx(1, "can't allocate program header %s", file);
                }
                ...
                for (iter = phdr; iter < &phdr[ehdr->e_phnum]; iter++) {
                        if (iter->p_type != PT_INTERP)
                                continue;
                        elf_interp = (char *)kmalloc(iter->p_filesz);
                        if (!elf_interp) {
                                z_close(fd);
                                kfree(phdr);
                                z_errx(1, "can't allocate interp segment");
                        }
                        if (z_lseek(fd, iter->p_offset, SEEK_SET) < 0) {
                                z_close(fd);
                                kfree(phdr);
        ...
        /* Free the interpreter path if it was allocated */
        if (elf_interp != NULL)
                kfree(elf_interp);
