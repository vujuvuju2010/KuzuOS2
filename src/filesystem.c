...
void fs_init() {
    ...
    print("=== FORCING ATAPI MODE (we booted from ISO) ===\n");
    ...
    if (detected == DEVICE_TYPE_NONE) {
        print_color("Detection failed, keeping ATAPI mode\n", VGA_COLOR_YELLOW);
    ...
    if (iso_get_volume_size_blocks(&iso_blocks) == 0) {
        ...
        print_color("No ISO detected or ISO read failed.\n", VGA_COLOR_YELLOW);
        print("Operating in RAM-only mode (no ISO preload).\n");
        
        // Initialize filesystem header in RAM
        fs_ensure_header_initialized();
        
        if (fs_disk_test() != 0) {
            print("Disk I/O test failed (expected without ISO).\n");
        }
        
        print_color("System ready (RAM-only mode)\n", VGA_COLOR_LIGHT_GREEN);
    ...
    print_color("System ready (ISO preloaded to RAM)\n", VGA_COLOR_LIGHT_GREEN);
}
