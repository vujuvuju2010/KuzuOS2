#include "vga.h"

// Forward declaration
static void draw_char(int x, int y, char c, uint8_t fg_color, uint8_t bg_color);

// VESA framebuffer variables - will be set by boot.asm
extern uint32_t* framebuffer;
extern uint32_t fb_width;
extern uint32_t fb_height;
extern uint32_t fb_pitch;

// Cursor position
static int cursor_x = 0;
static int cursor_y = 0;

// VGA color palette for 32-bit BGRA (common VESA format)
static uint32_t vga_colors[16] = {
    0xFF000000, // Black
    0xFFAA0000, // Blue  (BGR format)
    0xFF00AA00, // Green
    0xFFAAAA00, // Cyan  (BGR format)
    0xFF0000AA, // Red   (BGR format)
    0xFFAA00AA, // Magenta
    0xFF0055AA, // Brown (BGR format)
    0xFFAAAAAA, // Light Gray
    0xFF555555, // Dark Gray
    0xFFFF5555, // Light Blue (BGR format)
    0xFF55FF55, // Light Green
    0xFFFFFF55, // Light Cyan (BGR format)
    0xFF5555FF, // Light Red  (BGR format)
    0xFFFF55FF, // Light Magenta
    0xFF55FFFF, // Yellow (BGR format)
    0xFFFFFFFF  // White
};

// Classic IBM PC 8x8 font data
static uint8_t font_8x8[256][8] = {

        { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00},   // U+0000 (nul)
        { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00},   // U+0001
        { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00},   // U+0002
        { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00},   // U+0003
        { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00},   // U+0004
        { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00},   // U+0005
        { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00},   // U+0006
        { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00},   // U+0007
        { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00},   // U+0008
        { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00},   // U+0009
        { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00},   // U+000A
        { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00},   // U+000B
        { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00},   // U+000C
        { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00},   // U+000D
        { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00},   // U+000E
        { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00},   // U+000F
        { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00},   // U+0010
        { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00},   // U+0011
        { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00},   // U+0012
        { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00},   // U+0013
        { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00},   // U+0014
        { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00},   // U+0015
        { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00},   // U+0016
        { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00},   // U+0017
        { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00},   // U+0018
        { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00},   // U+0019
        { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00},   // U+001A
        { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00},   // U+001B
        { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00},   // U+001C
        { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00},   // U+001D
        { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00},   // U+001E
        { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00},   // U+001F
        { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00},   // U+0020 (space)
        { 0x18, 0x3C, 0x3C, 0x18, 0x18, 0x00, 0x18, 0x00},   // U+0021 (!)
        { 0x36, 0x36, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00},   // U+0022 (")
        { 0x36, 0x36, 0x7F, 0x36, 0x7F, 0x36, 0x36, 0x00},   // U+0023 (#)
        { 0x0C, 0x3E, 0x03, 0x1E, 0x30, 0x1F, 0x0C, 0x00},   // U+0024 ($)
        { 0x00, 0x63, 0x33, 0x18, 0x0C, 0x66, 0x63, 0x00},   // U+0025 (%)
        { 0x1C, 0x36, 0x1C, 0x6E, 0x3B, 0x33, 0x6E, 0x00},   // U+0026 (&)
        { 0x06, 0x06, 0x03, 0x00, 0x00, 0x00, 0x00, 0x00},   // U+0027 (')
        { 0x18, 0x0C, 0x06, 0x06, 0x06, 0x0C, 0x18, 0x00},   // U+0028 (()
        { 0x06, 0x0C, 0x18, 0x18, 0x18, 0x0C, 0x06, 0x00},   // U+0029 ())
        { 0x00, 0x66, 0x3C, 0xFF, 0x3C, 0x66, 0x00, 0x00},   // U+002A (*)
        { 0x00, 0x0C, 0x0C, 0x3F, 0x0C, 0x0C, 0x00, 0x00},   // U+002B (+)
        { 0x00, 0x00, 0x00, 0x00, 0x00, 0x0C, 0x0C, 0x06},   // U+002C (,)
        { 0x00, 0x00, 0x00, 0x3F, 0x00, 0x00, 0x00, 0x00},   // U+002D (-)
        { 0x00, 0x00, 0x00, 0x00, 0x00, 0x0C, 0x0C, 0x00},   // U+002E (.)
        { 0x60, 0x30, 0x18, 0x0C, 0x06, 0x03, 0x01, 0x00},   // U+002F (/)
        { 0x3E, 0x63, 0x73, 0x7B, 0x6F, 0x67, 0x3E, 0x00},   // U+0030 (0)
        { 0x0C, 0x0E, 0x0C, 0x0C, 0x0C, 0x0C, 0x3F, 0x00},   // U+0031 (1)
        { 0x1E, 0x33, 0x30, 0x1C, 0x06, 0x33, 0x3F, 0x00},   // U+0032 (2)
        { 0x1E, 0x33, 0x30, 0x1C, 0x30, 0x33, 0x1E, 0x00},   // U+0033 (3)
        { 0x38, 0x3C, 0x36, 0x33, 0x7F, 0x30, 0x78, 0x00},   // U+0034 (4)
        { 0x3F, 0x03, 0x1F, 0x30, 0x30, 0x33, 0x1E, 0x00},   // U+0035 (5)
        { 0x1C, 0x06, 0x03, 0x1F, 0x33, 0x33, 0x1E, 0x00},   // U+0036 (6)
        { 0x3F, 0x33, 0x30, 0x18, 0x0C, 0x0C, 0x0C, 0x00},   // U+0037 (7)
        { 0x1E, 0x33, 0x33, 0x1E, 0x33, 0x33, 0x1E, 0x00},   // U+0038 (8)
        { 0x1E, 0x33, 0x33, 0x3E, 0x30, 0x18, 0x0E, 0x00},   // U+0039 (9)
        { 0x00, 0x0C, 0x0C, 0x00, 0x00, 0x0C, 0x0C, 0x00},   // U+003A (:)
        { 0x00, 0x0C, 0x0C, 0x00, 0x00, 0x0C, 0x0C, 0x06},   // U+003B (;)
        { 0x18, 0x0C, 0x06, 0x03, 0x06, 0x0C, 0x18, 0x00},   // U+003C (<)
        { 0x00, 0x00, 0x3F, 0x00, 0x00, 0x3F, 0x00, 0x00},   // U+003D (=)
        { 0x06, 0x0C, 0x18, 0x30, 0x18, 0x0C, 0x06, 0x00},   // U+003E (>)
        { 0x1E, 0x33, 0x30, 0x18, 0x0C, 0x00, 0x0C, 0x00},   // U+003F (?)
        { 0x3E, 0x63, 0x7B, 0x7B, 0x7B, 0x03, 0x1E, 0x00},   // U+0040 (@)
        { 0x0C, 0x1E, 0x33, 0x33, 0x3F, 0x33, 0x33, 0x00},   // U+0041 (A)
        { 0x3F, 0x66, 0x66, 0x3E, 0x66, 0x66, 0x3F, 0x00},   // U+0042 (B)
        { 0x3C, 0x66, 0x03, 0x03, 0x03, 0x66, 0x3C, 0x00},   // U+0043 (C)
        { 0x1F, 0x36, 0x66, 0x66, 0x66, 0x36, 0x1F, 0x00},   // U+0044 (D)
        { 0x7F, 0x46, 0x16, 0x1E, 0x16, 0x46, 0x7F, 0x00},   // U+0045 (E)
        { 0x7F, 0x46, 0x16, 0x1E, 0x16, 0x06, 0x0F, 0x00},   // U+0046 (F)
        { 0x3C, 0x66, 0x03, 0x03, 0x73, 0x66, 0x7C, 0x00},   // U+0047 (G)
        { 0x33, 0x33, 0x33, 0x3F, 0x33, 0x33, 0x33, 0x00},   // U+0048 (H)
        { 0x1E, 0x0C, 0x0C, 0x0C, 0x0C, 0x0C, 0x1E, 0x00},   // U+0049 (I)
        { 0x78, 0x30, 0x30, 0x30, 0x33, 0x33, 0x1E, 0x00},   // U+004A (J)
        { 0x67, 0x66, 0x36, 0x1E, 0x36, 0x66, 0x67, 0x00},   // U+004B (K)
        { 0x0F, 0x06, 0x06, 0x06, 0x46, 0x66, 0x7F, 0x00},   // U+004C (L)
        { 0x63, 0x77, 0x7F, 0x7F, 0x6B, 0x63, 0x63, 0x00},   // U+004D (M)
        { 0x63, 0x67, 0x6F, 0x7B, 0x73, 0x63, 0x63, 0x00},   // U+004E (N)
        { 0x1C, 0x36, 0x63, 0x63, 0x63, 0x36, 0x1C, 0x00},   // U+004F (O)
        { 0x3F, 0x66, 0x66, 0x3E, 0x06, 0x06, 0x0F, 0x00},   // U+0050 (P)
        { 0x1E, 0x33, 0x33, 0x33, 0x3B, 0x1E, 0x38, 0x00},   // U+0051 (Q)
        { 0x3F, 0x66, 0x66, 0x3E, 0x36, 0x66, 0x67, 0x00},   // U+0052 (R)
        { 0x1E, 0x33, 0x07, 0x0E, 0x38, 0x33, 0x1E, 0x00},   // U+0053 (S)
        { 0x3F, 0x2D, 0x0C, 0x0C, 0x0C, 0x0C, 0x1E, 0x00},   // U+0054 (T)
        { 0x33, 0x33, 0x33, 0x33, 0x33, 0x33, 0x3F, 0x00},   // U+0055 (U)
        { 0x33, 0x33, 0x33, 0x33, 0x33, 0x1E, 0x0C, 0x00},   // U+0056 (V)
        { 0x63, 0x63, 0x63, 0x6B, 0x7F, 0x77, 0x63, 0x00},   // U+0057 (W)
        { 0x63, 0x63, 0x36, 0x1C, 0x1C, 0x36, 0x63, 0x00},   // U+0058 (X)
        { 0x33, 0x33, 0x33, 0x1E, 0x0C, 0x0C, 0x1E, 0x00},   // U+0059 (Y)
        { 0x7F, 0x63, 0x31, 0x18, 0x4C, 0x66, 0x7F, 0x00},   // U+005A (Z)
        { 0x1E, 0x06, 0x06, 0x06, 0x06, 0x06, 0x1E, 0x00},   // U+005B ([)
        { 0x03, 0x06, 0x0C, 0x18, 0x30, 0x60, 0x40, 0x00},   // U+005C (\)
        { 0x1E, 0x18, 0x18, 0x18, 0x18, 0x18, 0x1E, 0x00},   // U+005D (])
        { 0x08, 0x1C, 0x36, 0x63, 0x00, 0x00, 0x00, 0x00},   // U+005E (^)
        { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xFF},   // U+005F (_)
        { 0x0C, 0x0C, 0x18, 0x00, 0x00, 0x00, 0x00, 0x00},   // U+0060 (`)
        { 0x00, 0x00, 0x1E, 0x30, 0x3E, 0x33, 0x6E, 0x00},   // U+0061 (a)
        { 0x07, 0x06, 0x06, 0x3E, 0x66, 0x66, 0x3B, 0x00},   // U+0062 (b)
        { 0x00, 0x00, 0x1E, 0x33, 0x03, 0x33, 0x1E, 0x00},   // U+0063 (c)
        { 0x38, 0x30, 0x30, 0x3e, 0x33, 0x33, 0x6E, 0x00},   // U+0064 (d)
        { 0x00, 0x00, 0x1E, 0x33, 0x3f, 0x03, 0x1E, 0x00},   // U+0065 (e)
        { 0x1C, 0x36, 0x06, 0x0f, 0x06, 0x06, 0x0F, 0x00},   // U+0066 (f)
        { 0x00, 0x00, 0x6E, 0x33, 0x33, 0x3E, 0x30, 0x1F},   // U+0067 (g)
        { 0x07, 0x06, 0x36, 0x6E, 0x66, 0x66, 0x67, 0x00},   // U+0068 (h)
        { 0x0C, 0x00, 0x0E, 0x0C, 0x0C, 0x0C, 0x1E, 0x00},   // U+0069 (i)
        { 0x30, 0x00, 0x30, 0x30, 0x30, 0x33, 0x33, 0x1E},   // U+006A (j)
        { 0x07, 0x06, 0x66, 0x36, 0x1E, 0x36, 0x67, 0x00},   // U+006B (k)
        { 0x0E, 0x0C, 0x0C, 0x0C, 0x0C, 0x0C, 0x1E, 0x00},   // U+006C (l)
        { 0x00, 0x00, 0x33, 0x7F, 0x7F, 0x6B, 0x63, 0x00},   // U+006D (m)
        { 0x00, 0x00, 0x1F, 0x33, 0x33, 0x33, 0x33, 0x00},   // U+006E (n)
        { 0x00, 0x00, 0x1E, 0x33, 0x33, 0x33, 0x1E, 0x00},   // U+006F (o)
        { 0x00, 0x00, 0x3B, 0x66, 0x66, 0x3E, 0x06, 0x0F},   // U+0070 (p)
        { 0x00, 0x00, 0x6E, 0x33, 0x33, 0x3E, 0x30, 0x78},   // U+0071 (q)
        { 0x00, 0x00, 0x3B, 0x6E, 0x66, 0x06, 0x0F, 0x00},   // U+0072 (r)
        { 0x00, 0x00, 0x3E, 0x03, 0x1E, 0x30, 0x1F, 0x00},   // U+0073 (s)
        { 0x08, 0x0C, 0x3E, 0x0C, 0x0C, 0x2C, 0x18, 0x00},   // U+0074 (t)
        { 0x00, 0x00, 0x33, 0x33, 0x33, 0x33, 0x6E, 0x00},   // U+0075 (u)
        { 0x00, 0x00, 0x33, 0x33, 0x33, 0x1E, 0x0C, 0x00},   // U+0076 (v)
        { 0x00, 0x00, 0x63, 0x6B, 0x7F, 0x7F, 0x36, 0x00},   // U+0077 (w)
        { 0x00, 0x00, 0x63, 0x36, 0x1C, 0x36, 0x63, 0x00},   // U+0078 (x)
        { 0x00, 0x00, 0x33, 0x33, 0x33, 0x3E, 0x30, 0x1F},   // U+0079 (y)
        { 0x00, 0x00, 0x3F, 0x19, 0x0C, 0x26, 0x3F, 0x00},   // U+007A (z)
        { 0x38, 0x0C, 0x0C, 0x07, 0x0C, 0x0C, 0x38, 0x00},   // U+007B ({)
        { 0x18, 0x18, 0x18, 0x00, 0x18, 0x18, 0x18, 0x00},   // U+007C (|)
        { 0x07, 0x0C, 0x0C, 0x38, 0x0C, 0x0C, 0x07, 0x00},   // U+007D (})
        { 0x6E, 0x3B, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00},   // U+007E (~)
        { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00},    // U+007F

    // Fill remaining 128 slots with question marks or blanks
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0x80
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0x81
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0x82
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0x83
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0x84
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0x85
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0x86
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0x87
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0x88
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0x89
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0x8A
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0x8B
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0x8C
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0x8D
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0x8E
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0x8F
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0x90
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0x91
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0x92
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0x93
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0x94
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0x95
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0x96
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0x97
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0x98
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0x99
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0x9A
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0x9B
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0x9C
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0x9D
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0x9E
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0x9F
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0xA0
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0xA1
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0xA2
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0xA3
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0xA4
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0xA5
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0xA6
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0xA7
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0xA8
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0xA9
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0xAA
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0xAB
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0xAC
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0xAD
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0xAE
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0xAF
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0xB0
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0xB1
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0xB2
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0xB3
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0xB4
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0xB5
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0xB6
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0xB7
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0xB8
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0xB9
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0xBA
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0xBB
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0xBC
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0xBD
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0xBE
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0xBF
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0xC0
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0xC1
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0xC2
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0xC3
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0xC4
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0xC5
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0xC6
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0xC7
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0xC8
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0xC9
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0xCA
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0xCB
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0xCC
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0xCD
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0xCE
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0xCF
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0xD0
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0xD1
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0xD2
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0xD3
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0xD4
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0xD5
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0xD6
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0xD7
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0xD8
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0xD9
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0xDA
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0xDB
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0xDC
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0xDD
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0xDE
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0xDF
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0xE0
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0xE1
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0xE2
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0xE3
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0xE4
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0xE5
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0xE6
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0xE7
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0xE8
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0xE9
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0xEA
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0xEB
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0xEC
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0xED
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0xEE
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0xEF
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0xF0
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0xF1
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0xF2
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0xF3
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0xF4
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0xF5
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0xF6
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0xF7
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0xF8
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0xF9
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0xFA
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0xFB
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0xFC
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0xFD
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, // 0xFE
    {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}  // 0xFF
};

// Reverse bits in a byte to fix mirroring
static uint8_t reverse_bits(uint8_t b) {
    b = (b & 0xF0) >> 4 | (b & 0x0F) << 4;
    b = (b & 0xCC) >> 2 | (b & 0x33) << 2;
    b = (b & 0xAA) >> 1 | (b & 0x55) << 1;
    return b;
}



/// baba kral purna ansi time

// ANSI parser state
typedef enum {
    ANSI_STATE_NORMAL,
    ANSI_STATE_ESCAPE,
    ANSI_STATE_BRACKET,
    ANSI_STATE_PARAMS
} AnsiState;

static AnsiState ansi_state = ANSI_STATE_NORMAL;
static int ansi_params[16];
static int ansi_param_count = 0;
static int ansi_current_param = 0;
static uint8_t ansi_fg_color = VGA_COLOR_LIGHT_GREY;
static uint8_t ansi_bg_color = VGA_COLOR_BLACK;
static int ansi_reverse_video = 0;

// Helper to get actual fg/bg considering reverse video
static uint8_t get_actual_color(void) {
    if (ansi_reverse_video) {
        return (ansi_fg_color << 4) | ansi_bg_color;
    }
    return (ansi_bg_color << 4) | ansi_fg_color;
}

// Convert ANSI color code to VGA color
static uint8_t ansi_to_vga_color(int ansi_color) {
    switch (ansi_color) {
        case 30: case 40: return VGA_COLOR_BLACK;
        case 31: case 41: return VGA_COLOR_RED;
        case 32: case 42: return VGA_COLOR_GREEN;
        case 33: case 43: return VGA_COLOR_BROWN;
        case 34: case 44: return VGA_COLOR_BLUE;
        case 35: case 45: return VGA_COLOR_MAGENTA;
        case 36: case 46: return VGA_COLOR_CYAN;
        case 37: case 47: return VGA_COLOR_LIGHT_GREY;
        case 90: case 100: return VGA_COLOR_DARK_GREY;
        case 91: case 101: return VGA_COLOR_LIGHT_RED;
        case 92: case 102: return VGA_COLOR_LIGHT_GREEN;
        case 93: case 103: return VGA_COLOR_YELLOW;
        case 94: case 104: return VGA_COLOR_LIGHT_BLUE;
        case 95: case 105: return VGA_COLOR_LIGHT_MAGENTA;
        case 96: case 106: return VGA_COLOR_LIGHT_CYAN;
        case 97: case 107: return VGA_COLOR_WHITE;
        default: return VGA_COLOR_LIGHT_GREY;
    }
}

// Handle complete ANSI sequence
static void handle_ansi_sequence(char final_char) {
    int chars_per_line = fb_width / 8;
    int max_lines = fb_height / 16;
    
    switch (final_char) {
        case 'H': case 'f': // Cursor position
            {
                int row = (ansi_param_count > 0 && ansi_params[0] > 0) ? ansi_params[0] - 1 : 0;
                int col = (ansi_param_count > 1 && ansi_params[1] > 0) ? ansi_params[1] - 1 : 0;
                if (row >= max_lines) row = max_lines - 1;
                if (col >= chars_per_line) col = chars_per_line - 1;
                move_cursor(col, row);
            }
            break;
            
        case 'A': // Cursor up
            {
                int n = (ansi_param_count > 0 && ansi_params[0] > 0) ? ansi_params[0] : 1;
                int new_y = cursor_y - n;
                if (new_y < 0) new_y = 0;
                move_cursor(cursor_x, new_y);
            }
            break;
            
        case 'B': // Cursor down
            {
                int n = (ansi_param_count > 0 && ansi_params[0] > 0) ? ansi_params[0] : 1;
                int new_y = cursor_y + n;
                if (new_y >= max_lines) new_y = max_lines - 1;
                move_cursor(cursor_x, new_y);
            }
            break;
            
        case 'C': // Cursor forward
            {
                int n = (ansi_param_count > 0 && ansi_params[0] > 0) ? ansi_params[0] : 1;
                int new_x = cursor_x + n;
                if (new_x >= chars_per_line) new_x = chars_per_line - 1;
                move_cursor(new_x, cursor_y);
            }
            break;
            
        case 'D': // Cursor back
            {
                int n = (ansi_param_count > 0 && ansi_params[0] > 0) ? ansi_params[0] : 1;
                int new_x = cursor_x - n;
                if (new_x < 0) new_x = 0;
                move_cursor(new_x, cursor_y);
            }
            break;
            
        case 'J': // Clear screen
            {
                int mode = (ansi_param_count > 0) ? ansi_params[0] : 0;
                if (mode == 2) {
                    clear_screen();
                }
                // TODO: modes 0 and 1 (clear to end/beginning)
            }
            break;
            
        case 'K': // Clear line
            {
                int mode = (ansi_param_count > 0) ? ansi_params[0] : 0;
                uint8_t color = get_actual_color();
                uint8_t fg = color & 0x0F;
                uint8_t bg = (color >> 4) & 0x0F;
                
                if (mode == 0) { // Clear to end of line
                    for (int x = cursor_x; x < chars_per_line; x++) {
                        draw_char(x * 8, cursor_y * 16, ' ', fg, bg);
                    }
                } else if (mode == 1) { // Clear to beginning of line
                    for (int x = 0; x <= cursor_x; x++) {
                        draw_char(x * 8, cursor_y * 16, ' ', fg, bg);
                    }
                } else if (mode == 2) { // Clear entire line
                    for (int x = 0; x < chars_per_line; x++) {
                        draw_char(x * 8, cursor_y * 16, ' ', fg, bg);
                    }
                }
            }
            break;
            
        case 'm': // Graphics mode (colors)
            if (ansi_param_count == 0) {
                // Reset
                ansi_fg_color = VGA_COLOR_LIGHT_GREY;
                ansi_bg_color = VGA_COLOR_BLACK;
                ansi_reverse_video = 0;
            } else {
                for (int i = 0; i < ansi_param_count; i++) {
                    int param = ansi_params[i];
                    if (param == 0) {
                        // Reset
                        ansi_fg_color = VGA_COLOR_LIGHT_GREY;
                        ansi_bg_color = VGA_COLOR_BLACK;
                        ansi_reverse_video = 0;
                    } else if (param == 7) {
                        // Reverse video
                        ansi_reverse_video = 1;
                    } else if (param == 27) {
                        // Normal video
                        ansi_reverse_video = 0;
                    } else if (param >= 30 && param <= 37) {
                        // Foreground color
                        ansi_fg_color = ansi_to_vga_color(param);
                    } else if (param >= 40 && param <= 47) {
                        // Background color
                        ansi_bg_color = ansi_to_vga_color(param);
                    } else if (param >= 90 && param <= 97) {
                        // Bright foreground
                        ansi_fg_color = ansi_to_vga_color(param);
                    } else if (param >= 100 && param <= 107) {
                        // Bright background
                        ansi_bg_color = ansi_to_vga_color(param);
                    }
                }
            }
            break;
            
        case 'h': case 'l': // Set/reset mode
            // Handle cursor visibility if needed
            break;
    }
    
    // Reset state
    ansi_state = ANSI_STATE_NORMAL;
    ansi_param_count = 0;
    ansi_current_param = 0;
    for (int i = 0; i < 16; i++) ansi_params[i] = 0;
}

// Main ANSI-aware putchar function
void putchar_ansi(char c) {
    switch (ansi_state) {
        case ANSI_STATE_NORMAL:
            if (c == '\033') { // ESC
                ansi_state = ANSI_STATE_ESCAPE;
                ansi_param_count = 0;
                ansi_current_param = 0;
                for (int i = 0; i < 16; i++) ansi_params[i] = 0;
            } else {
                putchar_color(c, get_actual_color());
            }
            break;
            
        case ANSI_STATE_ESCAPE:
            if (c == '[') {
                ansi_state = ANSI_STATE_BRACKET;
            } else {
                // Invalid escape sequence, ignore and go back to normal
                ansi_state = ANSI_STATE_NORMAL;
            }
            break;
            
        case ANSI_STATE_BRACKET:
            if (c >= '0' && c <= '9') {
                ansi_current_param = ansi_current_param * 10 + (c - '0');
                ansi_state = ANSI_STATE_PARAMS;
            } else if (c == ';') {
                // Semicolon with no preceding number means 0
                if (ansi_param_count < 16) {
                    ansi_params[ansi_param_count++] = 0;
                }
                ansi_current_param = 0;
                // Stay in BRACKET state to handle next param
            } else {
                // Final character with no parameters
                handle_ansi_sequence(c);
            }
            break;
            
        case ANSI_STATE_PARAMS:
            if (c >= '0' && c <= '9') {
                ansi_current_param = ansi_current_param * 10 + (c - '0');
            } else if (c == ';') {
                if (ansi_param_count < 16) {
                    ansi_params[ansi_param_count++] = ansi_current_param;
                }
                ansi_current_param = 0;
                ansi_state = ANSI_STATE_BRACKET;
            } else {
                // Final character
                if (ansi_param_count < 16) {
                    ansi_params[ansi_param_count++] = ansi_current_param;
                }
                handle_ansi_sequence(c);
            }
            break;
    }
}

// Updated print function that uses ANSI parsing
void print_ansi(const char* str) {
    while (*str) {
        putchar_ansi(*str++);
    }
}

// allani sikim

static void draw_pixel(int x, int y, uint32_t color) {
    if (x < 0 || x >= (int)fb_width || y < 0 || y >= (int)fb_height || !framebuffer)
        return;
        
    uint32_t* pixel = framebuffer + y * (fb_pitch / 4) + x;
    *pixel = color;
}

// Draw an 8x16 character at pixel position (x, y) - like Linux TTY
static void draw_char(int x, int y, char c, uint8_t fg_color, uint8_t bg_color) {
    if (!framebuffer) return;
    
    uint32_t fg = vga_colors[fg_color & 0x0F];
    uint32_t bg = vga_colors[bg_color & 0x0F];
    
    // Ensure we use valid ASCII characters only
    uint8_t char_index = (uint8_t)c;
    if (char_index > 127) char_index = '?'; // Replace non-ASCII with ?
    
    uint8_t* glyph = font_8x8[char_index];
    
    uint32_t pitch_words = fb_pitch / 4;
    uint32_t* row_ptr = framebuffer + y * pitch_words + x;

    for (int row = 0; row < 8; row++) {
        uint8_t line = reverse_bits(glyph[row]); // FIXED: Now using reverse_bits!
        uint32_t* dst0 = row_ptr + (row * 2)     * pitch_words;
        uint32_t* dst1 = row_ptr + (row * 2 + 1) * pitch_words;
        for (int col = 0; col < 8; col++) {
            // Read bits MSB first (standard font format)
            uint32_t color = (line & (0x80 >> col)) ? fg : bg;
            // Draw each pixel twice vertically to make 8x16 (Linux TTY size)
            dst0[col] = color;
            dst1[col] = color;
        }
    }
}

void vga_init(uint32_t mb_magic, uint32_t mb_addr) {
    // VESA framebuffer is initialized in boot.asm
    // Reset cursor
    cursor_x = 0;
    cursor_y = 0;
}

void clear_screen() {
    if (!framebuffer) return;
    
    // Clear framebuffer to black
    uint32_t black = vga_colors[VGA_COLOR_BLACK];
    uint32_t pitch_words = fb_pitch / 4;
    for (uint32_t y = 0; y < fb_height; y++) {
        uint32_t* row = framebuffer + y * pitch_words;
        for (uint32_t x = 0; x < fb_width; x++) {
            row[x] = black;
        }
    }
    
    cursor_x = 0;
    cursor_y = 0;
}

void scroll() {
    if (!framebuffer) return;
    
    // Move framebuffer content up by 16 pixels (1 character row height)
    uint32_t pitch_words = fb_pitch / 4;
    uint32_t* src = framebuffer + pitch_words * 16;
    uint32_t* dst = framebuffer;
    uint32_t lines_to_copy = fb_height - 16;
    
    for (uint32_t y = 0; y < lines_to_copy; y++) {
        uint32_t* s = src + y * pitch_words;
        uint32_t* d = dst + y * pitch_words;
        for (uint32_t x = 0; x < fb_width; x++) {
            d[x] = s[x];
        }
    }
    
    // Clear the last 16 pixel rows
    uint32_t black = vga_colors[VGA_COLOR_BLACK];
    uint32_t* last_line = framebuffer + (fb_height - 16) * pitch_words;
    for (int y = 0; y < 16; y++) {
        uint32_t* row = last_line + y * pitch_words;
        for (uint32_t x = 0; x < fb_width; x++) {
            row[x] = black;
        }
    }
}

void move_cursor(int x, int y) {
    cursor_x = x;
    cursor_y = y;
}

int get_cursor_x() {
    return cursor_x;
}

int get_cursor_y() {
    return cursor_y;
}

void putchar_color(char c, uint8_t color) {
    if (!framebuffer) return;

    extern int process_may_use_console(void);
    if (!process_may_use_console()) {
        return;
    }
    
    int chars_per_line = fb_width / 8;    // 8 pixels wide per character (Linux TTY style)
    int max_lines = fb_height / 16;       // 16 pixels tall per character
    
    if (c == '\n') {
        cursor_x = 0;
        cursor_y++;
    } else if (c == '\r') {
        cursor_x = 0;
    } else if (c == '\b' || (unsigned char)c == 0x7F) { // backspace/delete visual erase
        if (cursor_x > 0) {
            cursor_x--;
            // draw a space at the current cell to clear
            uint8_t fg = color & 0x0F;
            uint8_t bg = (color >> 4) & 0x0F;
            draw_char(cursor_x * 8, cursor_y * 16, ' ', fg, bg);
        }
    } else if (c >= ' ') { // Printable character
        uint8_t fg = color & 0x0F;
        uint8_t bg = (color >> 4) & 0x0F;
        
        draw_char(cursor_x * 8, cursor_y * 16, c, fg, bg); // 8x16 character cells
        cursor_x++;
    }
    
    // Handle line wrapping
    if (cursor_x >= chars_per_line) {
        cursor_x = 0;
        cursor_y++;
    }
    
    // Handle scrolling
    if (cursor_y >= max_lines) {
        scroll();
        cursor_y = max_lines - 1;
    }
}

void putchar(char c) {
    putchar_color(c, VGA_COLOR_LIGHT_GREY);
}

void print(const char* str) {
    while (*str) {
        putchar(*str++);
    }
}

void print_color(const char* str, uint8_t color) {
    while (*str) {
        putchar_color(*str++, color);
    }
}

// Draw a bitmap image at position (x, y) with given width and height
// pixels should be RGBA32 format (BGRA in memory)
void vga_draw_bitmap(int x, int y, uint32_t width, uint32_t height, uint32_t* pixels) {
    if (!framebuffer || !pixels) return;
    
    // Calculate bounds for faster checking
    int x_start = (x < 0) ? 0 : x;
    int x_end = (x + (int)width > (int)fb_width) ? (int)fb_width : (x + (int)width);
    int y_start = (y < 0) ? 0 : y;
    int y_end = (y + (int)height > (int)fb_height) ? (int)fb_height : (y + (int)height);
    
    if (x_start >= x_end || y_start >= y_end) return;
    
    // Optimized pixel copying
    uint32_t pitch_words = fb_pitch / 4;
    for (int py = y_start; py < y_end; py++) {
        int src_y = py - y;
        uint32_t* src_row = pixels + src_y * width;
        uint32_t* dst_row = framebuffer + py * pitch_words;
        int src_x_start = x_start - x;
        int dst_x_start = x_start;
        int span = x_end - x_start;
        
        for (int i = 0; i < span; i++) {
            uint32_t pixel_color = src_row[src_x_start + i];
            
            // Check if pixel is transparent (alpha < 128)
            uint8_t alpha = (pixel_color >> 24) & 0xFF;
            if (alpha >= 128) {
                dst_row[dst_x_start + i] = pixel_color;
            }
        }
    }
}


